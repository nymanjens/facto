...
class InputWithDefaultFromReference[Value] private () {
  ...
  def apply[DelegateRef <: InputBase.Reference[Value]](
      ref: Reference,
  ...
  // **************** Public inner types ****************//
  case class InputElementExtraProps[DelegateRef <: InputBase.Reference[Value]](
      ref: DelegateRef,
  ...
  private final class Backend(val $ : BackendScope[Props.any, State]) {
    val implRef = Ref.toScalaComponent(Impl.component)
    val dummyRef = Ref.toScalaComponent(Dummy.component)
