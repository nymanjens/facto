  ...
  override def tests = TestSuite {
    ...
    "filters and sorts entries correctly" - {
      ...
      store.state ==> EntriesListStoreFactory.State
        .withGeneralEntries(hasMore = false, Seq(trans1, trans3), Seq(trans4, trans5))
    }

    "respects maxNumEntries" - {
      val store = factory.get("cats", maxNumEntries = 1)

      store.state ==> EntriesListStoreFactory.State.withGeneralEntries(hasMore = true, Seq(trans4, trans5))
    }
  }
}
